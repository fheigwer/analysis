...
start_off_image_analysis ("./EBImage_pipeline_Mamma.R",$infolder,$outfolder,24,"fld1wvDAPIDAPI.tif","fld1wvCy3Cy3.tif","fld1wvCy5Cy5.tif",30);
...
sub rename_tifs_in_folder {
	opendir dir, $_[0] or die print "folder ".$_[0]." notfound";
	foreach $file (sort(readdir(dir))){
		if($file=~m/(.+)\.tif/){
			...
			$name=~s/([\(\)])/\\$1/g;
			$newname=~s/\W//g;		
			rename "$dir/$name.tif","$dir/$newname.tif";
		}
	}
	closedir dir;
	return(1);      
				...
				if(	(-e $input_folder."/".$letter.$number.$nuclei_scheme) 
					...
					$sub_pm->start and next;
						#do the raw analysis either with R ,CP, or Hcell
							system("/usr/bin/R -f $path_to_script --slave --args ".$input_folder."/".$letter.$number.$nuclei_scheme.' '.$input_folder."/".$letter.$number.$body_scheme.' '.$input_folder."/".$letter.$number.$extra_scheme.' '.$output_folder.' '.$letter.$number); 

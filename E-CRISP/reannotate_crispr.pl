			...
			foreach my $hit (split(";;",${$CRISPR_hash{$query_id}}{"hits"})){
				if($hit ne ""){
					...
					if($hit_specs[5] eq "rc"){
						$crispr_seq=reverse_comp($db->seq($hit_specs[0],$hit_specs[1],($hit_specs[2]-1)));#+$something{"unspecific_leading_bases"}
						...
						$crispr_seq=$db->seq($hit_specs[0],$hit_specs[1]-$something{"unspecific_leading_bases"},$hit_specs[2]-1-$something{"unspecific_leading_bases"});#]-$something{"unspecific_leading_bases"}

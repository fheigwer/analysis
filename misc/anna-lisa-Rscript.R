...
clust=kmeans(anna.lisa.daten,3,nstart=10)
non_cor_data=anna.lisa.daten[,c(1,3,5,6)]
x=c()
y=c()
for(i in 1:10){
  clust=kmeans(non_cor_data,i,nstart=30)
  x=c(x,i)
  y=c(y,clust$betweenss/clust$totss )
}
clust=kmeans(non_cor_data,9,nstart=10)
 #tiff("pairs.tif",height=2024,width=2024)
 pairs(non_cor_data,col=clust$cluster)
 #dev.off()
